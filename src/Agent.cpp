...
Agent::Agent(const SpatialVector& pos, const SpatialVector& vel) : Thing(pos, vel)
{
//~ Agent::Agent(const SpatialVector& pos, const SpatialVector& vel, World* world) : Thing(pos, vel, world) {
	m_speed					= 1.5;
	m_friction				= 0.0;
	m_mass					= 100.0;
...
void Agent::perceive(World* world) {
	...
	// NOTE: For now, the Agent is confused by more than 10 objects
...
void Agent::logPerception(){
	...
	const unsigned int minNumberObjects = 30;
...
void Agent::comeUpWithPlan() {
	...
	for(std::list<ThingPtr>::iterator it = m_thingsInSight.begin(); it != m_thingsInSight.end(); ++it) {
		// std::cout << "Agent found an object of class " << (*it)->getClassID() << std::endl;
		// act on sheep
		if ((*it)->getClassID() == 5) {
...
void Agent::progress(double dt, World* world) {
...
void Agent::move(double dt, World* world) {

	m_velocity.normalize(m_speed);	// ANA: Agent has a constant speed
